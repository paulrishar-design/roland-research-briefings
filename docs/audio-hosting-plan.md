...
Measured 2026-09-02 against `main` @ `1c39863`; **re-measured 2026-09-05
against `main` @ `d6f6955` after the 2026-09-02/03 publish run.**

> **The deadline moved.** Three days after the first measurement, `public/` went
> from 370.4 MB (36.2% of the cap) to **818.2 MB — 79.9%**. Twelve new episodes
> landed on 2026-09-02/03 and several existing ones were re-encoded larger. The
> mean episode is now 21.0 MB, not 15.4 MB. **Headroom is about 9 more episodes,
> or 6 at worst-case size.** At the overall cadence that is ~13 days; at the rate
> of the 2026-09-02/03 burst (12 episodes in 2 days) it is under a week. Stage 1
> is no longer a planning exercise — it is the thing standing between this feed
> and a hard publish failure.

## Status

| Stage | State |
| --- | --- |
| Tooling + validator | **Done** — `validate_public.py` accepts both hosting layouts, `migrate_audio_to_release.py` automates stages 0–2, 23 tests cover both, all wired into CI (PR #2, green) |
...
stand-in audio at true byte sizes, at both 24 and 39 episodes. Validation
passes at every stage — before (39 on Pages), during the overlap (39 on
Releases, audio still on disk), and after removal — with every `<guid>`
unchanged.
...
| Measure | 2026-09-02 | 2026-09-05 |
| --- | --- | --- |
| `public/` total | 370.4 MB — 36.2% of cap | **818.2 MB — 79.9% of cap** |
| Episodes | 24, mean 15.4 MB (max 28.5) | **39, mean 21.0 MB (max 33.2)** |
| Headroom | 654 MB ≈ 42 episodes | **206 MB ≈ 9 episodes (6 worst-case)** |
| Publish cadence | 0.62/day | **0.76/day overall; 6.0/day in the last burst** |
| **Projected cap** | ~68 days | **~13 days, or under a week at burst rate** |
...
History already carried **61.7 MB of pure dead weight** as of 2026-09-02: 13
episode paths that exist nowhere in the current tree, left over from the
pre-`public/` layout (`episodes/alex-finn.mp3`) and from acceptance-test runs
(`episodes/alex-wissner-gross-standalone-acceptance.mp3`, 14.6 MB;
`episodes/youtube-miner-acceptance-handoff.mp3`, 8.0 MB). The 2026-09-02/03
re-encodes added more: every superseded copy of `dario-amodei.mp3`,
`david-sacks.mp3`, `ilya-sutskever.mp3`, `jensen-huang.mp3` and
`patrick-collison.mp3` is still in history alongside its replacement.

Secondary pressure, and no longer merely theoretical: every deploy re-uploads
the whole of `public/` as a Pages artifact. Runs took ~2 minutes at 370 MB. At
818 MB the six most recent deploys took **3m19s, 2m59s, 3m57s, 5m08s, 4m59s and
4m48s** — roughly half of the 10-minute Pages deploy timeout, and rising with
the tree. Stage 3 takes this back to seconds.
...
test one episode end-to-end in Apple Podcasts before backfilling the rest. Most
...
`public/` drops from 818 MB to ~1 MB. The Pages cap problem is now solved
...
today's 818 MB. With only ~9 episodes of headroom left, stage 1 is the one that
cannot wait — stage 2 can follow at leisure.
